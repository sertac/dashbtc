...
def calc_indicators(df):
    ...
    # ATR (Average True Range) - manuel hesaplama
    ...
    df["atr_ratio"]= df["atr"] / df["atr_ma"]  # ATR'nin MA'ya oranı (volatilite göstergesi)
...
def calculate_confluence_score(df, direction, htf_trend):
    """
    Ağırlıklı Confluence Score Sistemi — Min 4.0 puan, HTF alignment zorunlu
    """
    c = df.iloc[-1]; p = df.iloc[-2]
    ...
    # 1. HTF Trend Alignment (ZORUNLU - 2.0 puan)
    htf_aligned = (is_long and htf_trend == "BULL") or (not is_long and htf_trend == "BEAR")
    if htf_aligned:
        score += 2.0
        passed_keys.add("htf_aligned")
        checks.append({"label": f"✓ 1h HTF {htf_trend} aligned","status":"pass","side": "long" if is_long else "short"})
    elif htf_trend == "NEUTRAL":
        score += 0.5
        checks.append({"label": f"⚠ 1h HTF NÖTR (weak)","status":"warn","side":"neutral"})
    else:
        checks.append({"label": f"✗ 1h HTF {htf_trend} TERS!","status":"fail","side": "short" if is_long else "long"})

    # 2. EMA Setup (1.5 puan)
    cb = (p["ema_fast"] < p["ema_slow"]) and (c["ema_fast"] > c["ema_slow"])  # Bull cross
    cs = (p["ema_fast"] > p["ema_slow"]) and (c["ema_fast"] < c["ema_slow"])  # Bear cross
    ba = c["ema_fast"] > c["ema_slow"]  # Bull alignment
    sa = c["ema_fast"] < c["ema_slow"]  # Bear alignment

    if is_long:
        if cb:
            score += 1.5
            passed_keys.add("ema_cross")
            checks.append({"label": f"✓ EMA{EMA_FAST}>{EMA_SLOW} CROSS (+1.5)","status":"pass","side":"long"})
        elif ba:
            score += 0.8
            passed_keys.add("ema_aligned")
            checks.append({"label": f"→ EMA{EMA_FAST}>{EMA_SLOW} aligned (+0.8)","status":"pass","side":"long"})
        else:
            checks.append({"label": f"✗ EMA aleyhte","status":"fail","side":""})
    else:
        if cs:
            score += 1.5
            passed_keys.add("ema_cross")
            checks.append({"label": f"✓ EMA{EMA_FAST}<{EMA_SLOW} CROSS (+1.5)","status":"pass","side":"short"})
        elif sa:
            score += 0.8
            passed_keys.add("ema_aligned")
            checks.append({"label": f"→ EMA{EMA_FAST}<{EMA_SLOW} aligned (+0.8)","status":"pass","side":"short"})
        else:
            checks.append({"label": f"✗ EMA aleyhte","status":"fail","side":""})

    # 3. RSI Divergence / Extreme (2.0 puan — en güçlü sinyal)
    rsi = c["rsi"]
    div = False
    ...
    if div:
        score += 2.0
        passed_keys.add("rsi_divergence")
        checks.append({"label": f"✓ RSI DIVERGANS (+2.0)","status":"pass","side":"long" if is_long else "short"})
    elif is_long and rsi < RSI_OS:
        score += 1.0
        passed_keys.add("rsi_extreme")
        checks.append({"label": f"→ RSI aşırı satım {rsi:.0f} (+1.0)","status":"pass","side":"long"})
    elif not is_long and rsi > RSI_OB:
        score += 1.0
        passed_keys.add("rsi_extreme")
        checks.append({"label": f"→ RSI aşırı alım {rsi:.0f} (+1.0)","status":"pass","side":"short"})
    else:
        checks.append({"label": f"· RSI nötr {rsi:.0f}","status":"warn","side":"neutral"})

    # 4. Volume Spike (1.0 puan)
    vr = (c["volume"] / c["vol_ma"]) if c["vol_ma"] > 0 else 0
    if vr >= VOL_MULTIPLIER * 1.5:  # Güçlü spike
        score += 1.0
        passed_keys.add("volume_spike")
        checks.append({"label": f"✓ Hacim SPIKE ×{vr:.1f} (+1.0)","status":"pass","side":"neutral"})
    elif vr >= VOL_MULTIPLIER:
        score += 0.5
        passed_keys.add("volume_above")
        checks.append({"label": f"→ Hacim yüksek ×{vr:.1f} (+0.5)","status":"pass","side":"neutral"})
    else:
        checks.append({"label": f"· Hacim normal ×{vr:.1f}","status":"warn","side":"neutral"})

    # 5. Candle Pattern (1.5 puan)
    pat = detect_candle(df, is_long)
    if pat:
        score += 1.5
        passed_keys.add("candle_pattern")
        checks.append({"label": f"✓ {pat} (+1.5)","status":"pass","side":"long" if is_long else "short"})
    else:
        checks.append({"label": f"· Formasyon yok","status":"warn","side":"neutral"})

    # 6. ATR Volatilite Uygunluğu (0.5 puan)
    atr_ratio = c.get("atr_ratio", 1.0)
    if 0.8 <= atr_ratio <= 1.5:  # Normal volatilite = ideal
        score += 0.5
        passed_keys.add("atr_ok")
        checks.append({"label": f"→ ATR uygun ({atr_ratio:.2f}) (+0.5)","status":"pass","side":"neutral"})
    else:
        checks.append({"label": f"· ATR {atr_ratio:.2f}","status":"warn","side":"neutral"})

    # Confluence validation
    must_have = ["htf_aligned"]  # Zorunlu
    has_must_have = all(m in passed_keys for m in must_have)
    is_valid = has_must_have and score >= MIN_SCORE
    ...
    return {
        ...
        "is_valid": is_valid,
        "must_have_ok": has_must_have
    }

def score_reversal(df, direction):
    """
    Eski fonksiyon - confluence score wrapper (geriye uyumluluk)
    """
    global _htf_cache
    conf = calculate_confluence_score(df, direction, _htf_cache.get("trend", "NEUTRAL"))
    return conf["score"], conf["checks"]
...
def generate_signals(price, bid_walls, ask_walls, df):
    ...
    def _build(direction, wp, wv, dist):
        ...
        # Confluence Score hesapla (ağırlıklı sistem)
        conf = calculate_confluence_score(df, direction, htf["trend"])
        sc = conf["score"]
        ch = conf["checks"]

        # HTF kontrolü (confluence içinde zaten var ama ekstra kontrol)
        htf_ok = conf["must_have_ok"]

        # Market verisi ekle (confluence puanına dahil değil, ayrı değerlendirilir)
        mkt_sc, mkt_ch, mkt_hard = score_market_data(direction)

        # Market verisi çok kötüyse sinyali engelle (hard block)
        # Ama puanlama sistemine dahil etme (confluence kendi içinde yeterli)
        blocked = not htf_ok or mkt_hard or not conf["is_valid"]
        block_reason = []
        if not htf_ok:          block_reason.append(f"1h HTF ters")
        if mkt_hard:            block_reason.append("piyasa verisi")
        if not conf["is_valid"]: block_reason.append(f"skor düşük ({sc})")

        # Market checks'leri ekle (gösterim için)
        ch.extend(mkt_ch)
        ...
        return {
            ...
            "mkt_score": mkt_sc,
            "confluence": conf,  # Detaylı confluence verisi
        }
...
def background_loop():
    ...
    while True:
        try:
            ...
            candles_df=df.tail(60)[["open","high","low","close","volume","ema_fast","ema_slow","rsi","vol_ma","atr","atr_ratio"]].copy()
            candles_df[["ema_fast","ema_slow","rsi","vol_ma","atr","atr_ratio"]]=candles_df[["ema_fast","ema_slow","rsi","vol_ma","atr","atr_ratio"]].fillna(0)
            ...
            new_state={
                ...
                "atr_ratio":round(float(c["atr_ratio"]) if c.get("atr_ratio") else 0,2),
                "atr":round(float(c["atr"]) if c.get("atr") else 0,2),
                "atr_ratio":round(float(c["atr_ratio"]) if c.get("atr_ratio") is not None else 0,2),
                "atr":round(float(c["atr"]) if c.get("atr") is not None else 0,2),
...
<div class="grid">
  ...
  <div class="chart-panel">
    <div class="ind-grid">
      ...
      <div class="ind-card"><div class="ind-label">ATR (Volatilite)</div><div class="ind-value" id="ind-atr">—</div><div class="atr-bar-bg"><div class="atr-bar-fill" id="atr-fill" style="width:50%"></div></div></div>
    </div>
...
function renderIndicators(d){
  ...
  const at=d.atr||0,atr=d.atr_ratio||0;
  document.getElementById('ind-atr').textContent='$'+at.toFixed(0)+' (×'+atr.toFixed(1)+')';
  const atrClr=atr>1.5?'var(--red)':atr>1.2?'var(--amber)':'var(--green)';
  document.getElementById('ind-atr').style.color=atrClr;
  const af=document.getElementById('atr-fill');if(af){af.style.width=Math.min(100,atr*50)+'%';af.style.background=atrClr;}
}
