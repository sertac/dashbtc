    ...
    try:
        if os.path.exists(SIGNALS_FILE):
            with open(SIGNALS_FILE, 'r') as f:
                ...
                print(f"[SİSTEM] Yüklendi: {len(_pending_signals)} bekleyen, {len(_closed_signals)} kapalı")
...
def save_signals():
    try:
        with open(SIGNALS_FILE, 'w') as f:
            json.dump({'pending': _pending_signals, 'closed': _closed_signals}, f, indent=2)
    except Exception as e:
        print(f"[HATA] Kaydetme: {e}")
...
def generate_signals(price,bid_walls,ask_walls,df):
    signals=[]; htf=_htf_cache
    def _build(direction,wp,wv,dist):
        is_long=direction=="LONG"; sc,ch=score_reversal(df,direction)
        ...
        mkt_sc,mkt_ch,mkt_hard=score_market_data(direction); sc+=mkt_sc; ch+=mkt_ch
        ...
        rt=2*COMMISSION
        return {"dir":direction,"entry":price,"tp":tp,"sl":sl,
                "net_tp_pct":round((TP_PCT-rt)*100,2),"net_sl_pct":round((SL_PCT+rt)*100,2),
                "net_tp_usd":round(price*(TP_PCT-rt),2),"net_sl_usd":round(price*(SL_PCT+rt),2),
                "comm_usd":round(price*rt,2),"wall_price":wp,"wall_vol":round(wv,2),
                "dist_pct":round(dist*100,3),"score":sc,"checks":ch,
                "htf_blocked":blocked,"htf_trend":htf["trend"],
                "block_reason":" + ".join(block_reason) if block_reason else "","mkt_score":mkt_sc}
    ...
    for sig in _pending_signals:
        ...
        for _,row in future_candles.iterrows():
            ...
            if tp_hit and sl_hit:
                outcome="WIN" if (is_long and row["close"]>sig["entry"]) or (not is_long and row["close"]<sig["entry"]) else "LOSS"
            elif tp_hit: outcome="WIN"
            elif sl_hit: outcome="LOSS"
            else: continue
            rt=2*COMMISSION
            if outcome=="WIN": net_pnl_pct=round((TP_PCT-rt)*100,2); net_pnl_usd=round(sig["entry"]*(TP_PCT-rt),2)
            else: net_pnl_pct=-round((SL_PCT+rt)*100,2); net_pnl_usd=-round(sig["entry"]*(SL_PCT+rt),2)
            closed={**sig,"outcome":outcome,"net_pnl_pct":net_pnl_pct,"net_pnl_usd":net_pnl_usd,"close_ts":datetime.now().strftime("%H:%M:%S")}
            _closed_signals.append(closed); _closed_signals=_closed_signals[-100:]
            save_signals(); print(f"[SİNYAL] {sig['dir']} {outcome} | PnL: {net_pnl_pct}%")
            resolved=True; break
        sig["waited_count"]=sig.get("waited_count",0)+1
        if not resolved:
            if sig["waited_count"]>=MAX_CANDLES_WAIT:
                current=df.iloc[-1]["close"]; is_long=sig["dir"]=="LONG"
                outcome="WIN" if (is_long and current>sig["entry"]) or (not is_long and current<sig["entry"]) else "LOSS"
                rt=2*COMMISSION
                if outcome=="WIN": net_pnl_pct=round((TP_PCT-rt)*100,2); net_pnl_usd=round(sig["entry"]*(TP_PCT-rt),2)
                else: net_pnl_pct=-round((SL_PCT+rt)*100,2); net_pnl_usd=-round(sig["entry"]*(SL_PCT+rt),2)
                closed={**sig,"outcome":outcome,"net_pnl_pct":net_pnl_pct,"net_pnl_usd":net_pnl_usd,"close_ts":datetime.now().strftime("%H:%M:%S")+" (timeout)"}
                _closed_signals.append(closed); _closed_signals=_closed_signals[-100:]
                save_signals(); print(f"[SİNYAL TIMEOUT] {sig['dir']} {outcome}")
            else:
...
def calc_win_stats(closed_list=None):
    ...
    if not closed:
        return {"total":0,"wins":0,"losses":0,"win_rate":0,"long_total":0,"long_wins":0,"long_rate":0,
                "short_total":0,"short_wins":0,"short_rate":0,"net_pnl_pct":0,"net_pnl_usd":0,
                "comm_pct":round(2*COMMISSION*100,2)}
    wins=sum(1 for s in closed if s["outcome"]=="WIN"); losses=len(closed)-wins
    longs=[s for s in closed if s["dir"]=="LONG"]; shorts=[s for s in closed if s["dir"]=="SHORT"]
    lw=sum(1 for s in longs if s["outcome"]=="WIN"); sw=sum(1 for s in shorts if s["outcome"]=="WIN")
    return {"total":len(closed),"wins":wins,"losses":losses,"win_rate":round(wins/len(closed)*100,1),
...
def background_loop():
    ...
    while True:
        try:
            ...
            for sig in signals:
                already=any(p["dir"]==sig["dir"] and abs(p["entry"]-sig["entry"])<50 for p in _pending_signals)
                if not already:
                    _pending_signals.append({**sig,"ts":datetime.now().strftime("%H:%M:%S"),"symbol":SYMBOL,"entry_candle_idx":len(df)-1,"waited_count":0})
                    print(f"[YENİ SİNYAL] {sig['dir']} @ {sig['entry']} Score:{sig['score']}")
                    save_signals()
            ...
            new_state={
                ...
                "pending":[{k:v for k,v in s.items() if k not in ("checks","entry_candle_idx","waited_count")}
...
<div class="grid">
  ...
  <div class="panel" style="grid-row:1;overflow-y:auto">
    ...
    <div class="panel-title" style="margin-top:10px">Aktif Sinyaller</div>
...
function renderSignals(d){
  ...
  const active=(d.signals||[]).filter(s=>!s.htf_blocked),blocked=(d.signals||[]).filter(s=>s.htf_blocked);
...
function renderPending(d){
  ...
  area.innerHTML=d.pending.map(s=>{const isLong=s.dir==='LONG',clr=isLong?'var(--green)':'var(--red)';return`<div class="pending-row"><span class="badge pending">${s.dir}</span><span style="color:${clr};font-size:11px">$${fmt(s.entry)}</span><span style="color:var(--text-dim);font-size:9px">TP <span style="color:var(--green)">$${fmt(s.tp)}</span> SL <span style="color:var(--red)">$${fmt(s.sl)}</span></span><span style="color:var(--text-dim);font-size:9px">${s.ts||''}</span></div>`;}).join('');
}

function renderClosed(d){
  const area=document.getElementById('closed-area');
  if(!d.closed.length){area.innerHTML='<div style="color:var(--text-dim);font-size:10px;padding:5px 0">Henüz kapanmadı</div>';return;}
  area.innerHTML=d.closed.map(s=>{const isLong=s.dir==='LONG',isWin=s.outcome==='WIN',pnlPct=s.net_pnl_pct||0,pnlSign=pnlPct>=0?'+':'';
    return`<div class="result-row"><span class="badge ${isWin?'win':'loss'}">${isWin?'✓ WIN':'✗ LOSS'}</span><span style="color:${isLong?'var(--green)':'var(--red)'};">${s.dir}</span><span style="font-size:9px;color:var(--text-dim)">${fmt(s.entry)}</span><span style="font-size:10px;color:${isWin?'var(--green)':'var(--red)'}">${pnlSign}${pnlPct.toFixed(2)}%</span><span style="color:var(--amber)">${stars(s.score)}</span><span style="color:var(--text-dim);font-size:9px">${s.close_ts||s.ts||''}</span></div>`;}).join('');
}
...
src.onmessage=e=>{
  ...
  if(d.symbol){const sel=document.getElementById('symbol-select');if(sel&&sel.value!==d.symbol)sel.value=d.symbol;const lbl=document.getElementById('h-symbol-label');if(lbl){const p=d.symbol.split('/');lbl.innerHTML=p[0]+'<span>/'+( p[1]||'USDT')+'</span> · SİNYAL BOTU';}}
