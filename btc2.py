"""
BTC/USDT Sinyal Botu — Tek Dosya Flask Dashboard
==================================================
Kurulum:
  pip install flask ccxt pandas ta
...
def detect_candle(df, is_long):
    ...
    if is_long:
        ...
        if len(df) >= 3:
            pp = df.iloc[-3]
            if pp["body"] < 0 and abs(p["body"]) < abs(pp["body"])*0.4 and b > 0:
                return "Morning Star ⭐"
...
def score_reversal(df, direction):
    ...
    if is_long and (cb or ba):
        score += 1
        checks.append({"label": f"EMA{EMA_FAST}>{EMA_SLOW}" + (" kesişim" if cb else ""), "status":"pass","side":"long"})
    elif not is_long and (cs or not ba):
        score += 1
        checks.append({"label": f"EMA{EMA_FAST}<{EMA_SLOW}" + (" kesişim" if cs else ""), "status":"pass","side":"short"})
    else:
    ...
    if is_long and (rsi < RSI_OS or div):
        score += 1
        checks.append({"label": f"RSI {'diverjans' if div else 'aşırı satım'} ({rsi:.0f})","status":"pass","side":"long"})
    elif not is_long and (rsi > RSI_OB or div):
        score += 1
        checks.append({"label": f"RSI {'diverjans' if div else 'aşırı alım'} ({rsi:.0f})","status":"pass","side":"short"})
    else:
    ...
    if vr >= VOL_MULTIPLIER:
        score += 1
        checks.append({"label": f"Hacim spike ×{vr:.1f}","status":"pass","side":"neutral"})
    else:
    ...
    if pat:
        score += 1
        checks.append({"label": pat,"status":"pass","side":"long" if is_long else "short"})
    else:
...
def background_loop():
    ...
    while True:
        try:
            ...
            for sig in signals:
                entry = {**sig, "ts": datetime.now().strftime("%H:%M:%S")}
                if not any(h["dir"]==sig["dir"] and abs(h["entry"]-sig["entry"])<50
                           for h in _signal_history[-3:] if _signal_history):
                    _signal_history.append(entry)
                    _signal_history = _signal_history[-50:]
            new_state["signal_history"] = _signal_history[-20:]
...
:root {
  ...
  --green:#00d264; --green-dim:#00431f;
  --red:#ff3d5a;   --red-dim:#6b0018;
...
/* Header */
header{display:flex;align-items:center;gap:20px;padding:10px 20px;background:var(--bg2);
  border-bottom:1px solid var(--border);position:sticky;top:0;z-index:100;flex-wrap:wrap}
.logo{font-size:14px;font-weight:600;letter-spacing:.12em;color:var(--amber);text-transform:uppercase;white-space:nowrap}
.logo span{color:var(--text-dim);font-weight:300}
.hstat{display:flex;flex-direction:column;gap:1px;border-left:1px solid var(--border);padding-left:16px}
.hstat-label{font-size:9px;letter-spacing:.1em;color:var(--text-dim);text-transform:uppercase}
.hstat-val{font-size:14px;font-weight:500}
.price-big{font-size:24px;font-weight:600;color:var(--amber);letter-spacing:.04em}
...
.dot-live{width:8px;height:8px;border-radius:50%;background:var(--green);
  box-shadow:0 0 6px var(--green);animation:blink 1.4s infinite;margin-left:auto}
@keyframes blink{0%,100%{opacity:1}50%{opacity:.2}}
.ts{font-size:11px;color:var(--text-dim)}

/* Grid */
.grid{display:grid;grid-template-columns:320px 1fr 290px;gap:1px;background:var(--border);
  min-height:calc(100vh - 52px)}
.panel{background:var(--bg2);padding:14px 16px;overflow:hidden}
...
.ob-table td{padding:3px 4px;font-size:12px;white-space:nowrap;border-bottom:1px solid rgba(255,255,255,.02)}
.bar-bg{height:7px;border-radius:2px;background:var(--border);overflow:hidden}
.bar-fill{height:100%;border-radius:2px;transition:width .4s}
.bar-ask{background:var(--red)} .bar-bid{background:var(--green)}
.ob-divider td{padding:6px 4px;font-size:13px;font-weight:600;color:var(--amber);background:var(--bg3);
...
.ind-card{background:var(--bg2);padding:10px 14px;display:flex;flex-direction:column;gap:3px}
.ind-label{font-size:9px;letter-spacing:.12em;text-transform:uppercase;color:var(--text-dim)}
.ind-value{font-size:18px;font-weight:500}
.rsi-bar-bg{height:4px;background:var(--bg3);border-radius:2px;margin-top:4px;position:relative;overflow:visible}
...
.chart-wrap{background:var(--bg2);padding:14px 16px;flex:1;display:flex;flex-direction:column;min-height:320px}
canvas{flex:1}

/* Signals */
.signal-box{background:var(--bg3);border-radius:4px;padding:12px 14px;margin-bottom:8px;
...
.sig-dir{font-size:13px;font-weight:600;letter-spacing:.06em}
.sig-score{font-size:11px;color:var(--text-dim)}
.sig-levels{display:grid;grid-template-columns:1fr 1fr 1fr;gap:4px;margin-bottom:8px}
...
.check-item{font-size:10px;display:flex;align-items:center;gap:5px}
.stars{letter-spacing:2px;font-size:13px}
.star-fill{color:var(--amber)} .star-empty{color:var(--border)}
.no-signal{color:var(--text-dim);font-size:11px;text-align:center;padding:24px 0;
  border:1px dashed var(--border);border-radius:4px}
.no-signal-big{font-size:22px;margin-bottom:6px}

/* History */
.history-row{display:flex;align-items:center;gap:8px;padding:5px 0;
  border-bottom:1px solid var(--border);font-size:11px}
.history-row:last-child{border:none}
.hist-dir{font-weight:600;width:48px}
.hist-price{flex:1;color:var(--text-dim)}
.hist-score{color:var(--amber)}
.hist-ts{color:var(--text-dim);font-size:10px}
...
<header>
  ...
  <div class="hstat">
    <div class="hstat-label">Fiyat</div>
    <div class="price-big" id="h-price">—</div>
  </div>
  <div class="hstat">
    <div class="hstat-label">24s Değişim</div>
    <div class="hstat-val" id="h-change">—</div>
  </div>
  <div class="hstat">
    <div class="hstat-label">RSI (14)</div>
    <div class="hstat-val" id="h-rsi">—</div>
  </div>
  <div class="hstat">
    <div class="hstat-label">EMA 9/21</div>
    <div class="hstat-val" id="h-ema">—</div>
  </div>
  <div class="hstat">
    <div class="hstat-label">Aktif Sinyal</div>
    <div class="hstat-val" id="h-sigcount">—</div>
  </div>
...
<div class="grid">
  ...
  <!-- Sol: Order Book -->
  <div class="panel" style="grid-row:1/3;overflow-y:auto;max-height:calc(100vh - 52px)">
  ...
  <div class="panel" style="grid-row:1/3;overflow-y:auto;max-height:calc(100vh - 52px)">
    ...
    <div id="signal-area">
      <div class="no-signal"><div class="no-signal-big">⏳</div>Veri bekleniyor…</div>
    </div>
    <div class="panel-title" style="margin-top:20px">Sinyal Geçmişi</div>
    <div id="history-area">
      <div style="color:var(--text-dim);font-size:11px;text-align:center;padding:12px 0">Henüz sinyal yok</div>
    </div>
  </div>
  ...
  chart = new Chart(ctx, {
    ...
    options:{
      ...
      plugins:{
        ...
        tooltip:{ callbacks:{ label:(c)=>{const d=c.raw;return[`O:$${d.o.toLocaleString()}`,`H:$${d.h.toLocaleString()}`,`L:$${d.l.toLocaleString()}`,`C:$${d.c.toLocaleString()}`]} } }
      },
      scales:{
        ...
        y:{ position:'right', grid:{color:'rgba(30,45,58,.6)'},
            ticks:{color:'#4a6070',font:{family:'IBM Plex Mono',size:10},callback:v=>'$'+v.toLocaleString()} }
      }
    }
  });
}
...
function renderHeader(d) {
  ...
  const emaBull=d.ema_fast>d.ema_slow;
  document.getElementById('h-ema').innerHTML=`<span class="${emaBull?'up':'down'}">${emaBull?'BULL ▲':'BEAR ▼'}</span>`;
  document.getElementById('h-sigcount').textContent=d.signals.length||'—';
...
function renderOrderBook(d) {
  ...
  [...d.ask_walls].reverse().forEach(w=>{
    ...
    html+=`<tr>
      <td style="color:var(--red)">$${fmt(w.price)} <small style="color:var(--text-dim)">+${dist}%${near?' ⚡':''}</small></td>
      <td style="color:var(--red);text-align:right">${w.vol.toFixed(1)} BTC</td>
      <td style="padding-left:8px"><div class="bar-bg"><div class="bar-fill bar-ask" style="width:${bp}%"></div></div></td>
    </tr>`;
  });
  html+=`<tr class="ob-divider"><td>▶ $${fmt(price)}</td><td colspan="2" style="color:var(--text-dim);font-size:11px;text-align:right">SPOT</td></tr>`;
  [...d.bid_walls].reverse().forEach(w=>{
    const dist=((price-w.price)/price*100).toFixed(2);
    const bp=(w.vol/maxVol*100).toFixed(1);
    const near=parseFloat(dist)<=PROXIMITY_PCT*100;
    html+=`<tr>
      <td style="color:var(--green)">$${fmt(w.price)} <small style="color:var(--text-dim)">-${dist}%${near?' ⚡':''}</small></td>
      <td style="color:var(--green);text-align:right">${w.vol.toFixed(1)} BTC</td>
      <td style="padding-left:8px"><div class="bar-bg"><div class="bar-fill bar-bid" style="width:${bp}%"></div></div></td>
    </tr>`;
  });
...
function renderSignals(d) {
  ...
  if(!d.signals.length){
    area.innerHTML='<div class="no-signal"><div class="no-signal-big">⏳</div>Sinyal yok — duvar yakınlığı veya teyit skoru yetersiz</div>';
    return;
  }
...
function renderHistory(d) {
  if(!d.signal_history||!d.signal_history.length) return;
  document.getElementById('history-area').innerHTML=[...d.signal_history].reverse().map(h=>{
    const isLong=h.dir==='LONG';
    return `<div class="history-row">
      <span class="hist-dir" style="color:${isLong?'var(--green)':'var(--red)'}">${h.dir}</span>
      <span class="hist-price">$${fmt(h.entry)}</span>
      <span class="hist-score">${stars(h.score)}</span>
...
@app.route("/")
def index():
    # Python sabitlerini JS değişkenlerine yerleştir
    html = HTML \
