...
/* ── Chart ── */
.chart-panel{grid-column:2;grid-row:1;display:flex;flex-direction:column;gap:1px;min-height:0}
.chart-wrap{background:var(--bg2);padding:12px 14px 8px;flex:1;display:flex;
...
<div class="grid">
  ...
  <div class="chart-panel">
    ...
    <div class="chart-wrap">
      ...
      <div id="chart-container" style="flex:1;min-height:0;position:relative;overflow:hidden">
        <canvas id="price-chart" style="position:absolute;top:0;left:0;cursor:crosshair"></canvas>
      </div>
    </div>
  </div>
...
function drawChart(candles) {
  ...
  // Henüz boyutlandırılmadıysa tekrar dene
  if (!W || !H || W < 50 || H < 50) {
    setTimeout(() => drawChart(candles), 100);
    return;
  }
...
function renderTweets(d) {
  ...
  if (!d.tweet_kw || !d.tweet_kw.length) {
    list.innerHTML = '<div style="color:var(--text-dim);font-size:11px;text-align:center;padding:20px 0">Kelime girerek aramayı başlatın</div>';
    return;
  }
  if (!d.tweets || !d.tweets.length) {
    list.innerHTML = '<div style="color:var(--text-dim);font-size:11px;text-align:center;padding:20px 0">⚠ Nitter erişilemedi veya sonuç yok.<br>Başka instance deneniyor…</div>';
    return;
  }
  list.innerHTML = d.tweets.map(t =>
    `<div class="tweet-item">
      ...
      <div class="tweet-time">${escHtml(t.ts||'')}
        ${t.url ? `<a href="${encodeURI(t.url)}" target="_blank" rel="noopener" style="color:var(--text-dim);font-size:9px;text-decoration:none;margin-left:6px">→ aç</a>` : ''}
      </div>
    </div>`
  ).join('');
}
