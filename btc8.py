...
def fetch_market_data():
    ...
    4 kaynaktan veri çeker:
      ...
      4. Taker Hacmi    — /futures/data/takerbuybasevol
    """
    sym    = "BTCUSDT"
    ...
    # 4. Taker Buy/Sell Hacmi (son 3 × 5dk)
    try:
        tk_data = _get("/futures/data/takerbuybasevol",
                       {"symbol": sym, "period": "5m", "limit": 3})
        if tk_data:
            buy_vol  = sum(float(r["buyVol"])  for r in tk_data)
            sell_vol = sum(float(r["sellVol"]) for r in tk_data)
            tk_ratio = buy_vol / sell_vol if sell_vol > 0 else 1.0
            ...
            result["taker_buy"]   = round(buy_vol, 2)
            result["taker_sell"]  = round(sell_vol, 2)
...
.tab-btn{flex:1;padding:9px 4px;background:none;border:none;border-bottom:2px solid transparent;
  color:var(--text-dim);font-family:var(--mono);font-size:10px;letter-spacing:.06em;
...
<div class="grid">
  ...
  <!-- Sol: 3 Sekmeli Karar Merkezi -->
  <div class="panel" style="grid-row:1/3;overflow-y:hidden;max-height:calc(100vh - 50px);display:flex;flex-direction:column;padding:0">

    <!-- Sekme başlıkları -->
    <div style="display:flex;border-bottom:1px solid var(--border);flex-shrink:0">
      <button class="tab-btn active" onclick="switchTab('analiz')" id="tab-analiz">📊 Analiz</button>
      <button class="tab-btn" onclick="switchTab('haberler')" id="tab-haberler">📰 Haberler</button>
      <button class="tab-btn" onclick="switchTab('tweetler')" id="tab-tweetler">𝕏 Tweetler</button>
    </div>

    <!-- ══ SEKME 1: ANALİZ ══ -->
    <div id="pane-analiz" class="tab-pane" style="overflow-y:auto;flex:1;padding:12px 14px">
      ...
      <div id="tech-panel">
        <div class="dec-row" id="dec-ema">
          <div class="dec-icon" id="dec-ema-icon">·</div>
          <div class="dec-body"><div class="dec-label">EMA 9 / 21</div><div class="dec-val" id="dec-ema-val">—</div></div>
          <div class="dec-bar-wrap"><div class="dec-bar" id="dec-ema-bar"></div></div>
        </div>
        <div class="dec-row" id="dec-rsi">
          <div class="dec-icon" id="dec-rsi-icon">·</div>
          <div class="dec-body"><div class="dec-label">RSI (14)</div><div class="dec-val" id="dec-rsi-val">—</div></div>
          <div class="dec-bar-wrap">
            <div class="dec-bar" id="dec-rsi-bar" style="width:50%"></div>
        ...
        <div class="dec-row" id="dec-vol">
          <div class="dec-icon" id="dec-vol-icon">·</div>
          <div class="dec-body"><div class="dec-label">Hacim Spike</div><div class="dec-val" id="dec-vol-val">—</div></div>
          <div class="dec-bar-wrap"><div class="dec-bar" id="dec-vol-bar"></div></div>
        </div>
        <div class="dec-row" id="dec-candle">
          <div class="dec-icon" id="dec-candle-icon">·</div>
          <div class="dec-body"><div class="dec-label">Mum Formasyonu</div><div class="dec-val" id="dec-candle-val">—</div></div>
          <div class="dec-bar-wrap" style="width:20px"></div>
        </div>
      </div>

      <!-- 1h HTF -->
      <div class="panel-title" style="margin-top:14px">1h HTF Trend <span id="htf-badge" style="margin-left:4px"></span></div>
      ...
      <!-- Piyasa Verisi -->
      <div class="panel-title" style="margin-top:14px">Piyasa Verisi
        <span id="mkt-ts" style="color:var(--text-dim);font-size:9px;margin-left:4px"></span>
      </div>
      <div class="dec-row" id="dec-fr">
        <div class="dec-icon" id="dec-fr-icon">·</div>
        <div class="dec-body"><div class="dec-label">Funding Rate</div><div class="dec-val" id="dec-fr-val">—</div></div>
        <div class="dec-bar-wrap"><div class="dec-bar" id="dec-fr-bar" style="width:50%"></div></div>
      </div>
      <div class="dec-row" id="dec-oi">
        <div class="dec-icon" id="dec-oi-icon">·</div>
        <div class="dec-body"><div class="dec-label">Open Interest</div><div class="dec-val" id="dec-oi-val">—</div></div>
        <div class="dec-bar-wrap"><div class="dec-bar" id="dec-oi-bar" style="width:50%"></div></div>
      </div>
      <div class="dec-row" id="dec-ls">
        <div class="dec-icon" id="dec-ls-icon">·</div>
        <div class="dec-body"><div class="dec-label">Long / Short Oranı</div><div class="dec-val" id="dec-ls-val">—</div></div>
        <div class="dec-bar-wrap"><div class="dec-bar" id="dec-ls-bar" style="width:50%"></div></div>
      </div>
      <div class="dec-row" id="dec-tk">
        <div class="dec-icon" id="dec-tk-icon">·</div>
        <div class="dec-body"><div class="dec-label">Taker Buy / Sell</div><div class="dec-val" id="dec-tk-val">—</div></div>
        <div class="dec-bar-wrap"><div class="dec-bar" id="dec-tk-bar" style="width:50%"></div></div>
      </div>
    ...
    <!-- ══ SEKME 2: HABERLER ══ -->
    <div id="pane-haberler" class="tab-pane" style="display:none;overflow-y:auto;flex:1;padding:12px 14px">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px">
        <span style="font-size:9px;color:var(--text-dim);letter-spacing:.1em;text-transform:uppercase">
          5 kaynak · 2dk yenileme
        </span>
        <button onclick="refreshNews()" style="font-size:9px;color:var(--amber);background:none;border:1px solid var(--amber-dim);border-radius:3px;padding:2px 8px;cursor:pointer">
          ↻ Yenile
        </button>
      </div>
      <div id="news-list">
        <div style="color:var(--text-dim);font-size:11px;text-align:center;padding:30px 0">
          Haberler yükleniyor…
        </div>
      </div>
    </div>
    ...
    <!-- ══ SEKME 3: TWEETLER ══ -->
    <div id="pane-tweetler" class="tab-pane" style="display:none;overflow-y:auto;flex:1;padding:12px 14px">
      <!-- Keyword input -->
      <div style="margin-bottom:10px">
        <div style="font-size:9px;color:var(--text-dim);text-transform:uppercase;letter-spacing:.1em;margin-bottom:5px">
          Anahtar Kelimeler <span style="color:var(--text-dim)">(virgülle ayır)</span>
        </div>
        <div style="display:flex;gap:6px">
          ...
          <button onclick="setKeywords()"
            style="background:var(--amber-dim);color:var(--amber);border:none;border-radius:4px;
                   padding:5px 10px;cursor:pointer;font-family:var(--mono);font-size:11px;white-space:nowrap">
            Ara
          </button>
        </div>
        <div id="kw-tags" style="display:flex;gap:4px;flex-wrap:wrap;margin-top:5px"></div>
      </div>
      <div style="font-size:9px;color:var(--text-dim);margin-bottom:8px">
        Nitter RSS · 90sn yenileme
      </div>
      <div id="tweet-list">
        <div style="color:var(--text-dim);font-size:11px;text-align:center;padding:30px 0">
          Kelime girerek aramayı başlatın
        </div>
      </div>
    </div>
...
function switchTab(name) {
  ['analiz','haberler','tweetler'].forEach(t => {
    document.getElementById('pane-' + t).style.display = t === name ? 'flex' : 'none';
    document.getElementById('tab-' + t).classList.toggle('active', t === name);
  });
  // flex yerine block kullan analiz için
  if (name === 'analiz') document.getElementById('pane-analiz').style.display = 'block';
}
