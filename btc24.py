...
def db_init():
    ...
    with db_connect() as conn:
        ...
        for col, typ in [('exit_price','REAL'), ('duration_min','INTEGER')]:
            try:
...
def db_close_signal(rowid, outcome, net_pnl_pct, net_pnl_usd, close_ts, exit_price=None, duration_min=None):
    status = outcome.lower()
    with db_connect() as conn:
        conn.execute("""
            UPDATE signals SET status=?, outcome=?, net_pnl_pct=?, net_pnl_usd=?,
                               close_ts=?, exit_price=?, duration_min=?
            WHERE id=?
        """, (status, outcome, net_pnl_pct, net_pnl_usd, close_ts, exit_price, duration_min, rowid))
        conn.commit()
...
def _close_signal(sig, outcome, net_pnl_pct, net_pnl_usd, close_ts, exit_price=None):
    """Sinyali hem DB'ye hem RAM cache'e yaz."""
    global _closed_signals
    # Süre hesapla (dakika)
    duration_min = None
    try:
        waited = sig.get("waited_count", 0)
        duration_min = waited * 5  # her waited_count = 1 REFRESH_SEC * 5dk mum
    except Exception:
        pass
    ...
    print(f"[SİNYAL] {sig['dir']} {outcome} | PnL:{net_pnl_pct}% | Çıkış:{exit_price} | Süre:{duration_min}dk | DB:{db_id}")
...
def check_pending_signals(df):
    ...
    for sig in _pending_signals:
        ...
        for _, row in future_candles.iterrows():
            ...
            _close_signal(sig, outcome, net_pnl_pct, net_pnl_usd, now_ts, exit_price)
            resolved = True
            break
        ...
        if not resolved:
            if sig["waited_count"] >= MAX_CANDLES_WAIT:
                # Timeout: TP/SL'e ulaşamadı.
                # Gerçek P&L = (mevcut fiyat - giriş) / giriş × yön − komisyon
                current = float(df.iloc[-1]["close"])
                is_long = sig["dir"] == "LONG"
                raw_pct = (current - sig["entry"]) / sig["entry"] * (1 if is_long else -1)
                net_pnl_pct = round((raw_pct - rt) * 100, 2)
                net_pnl_usd = round(sig["entry"] * (raw_pct - rt), 2)
                outcome     = "WIN" if net_pnl_pct > 0 else "LOSS"
                _close_signal(sig, outcome, net_pnl_pct, net_pnl_usd,
                              now_ts + f" ⏱timeout @{current:.0f}", exit_price=current)
            else:
  ...
  area.innerHTML=d.closed.map(s=>{
    ...
    return`<div style="background:var(--bg3);border-radius:4px;padding:7px 10px;margin-bottom:5px;border-left:3px solid ${pnlClr}">
      ...
      <div style="font-size:9px;color:var(--text-dim);display:flex;gap:8px">
        <span>📅 ${openTs}</span>
        <span>→ ${closeTs}</span>
        <span style="margin-left:auto">${s.htf_trend||s.htf_blocked?'1h '+(s.htf_trend||'?'):''}</span>
      </div>
    </div>`;
  }).join('');
}
