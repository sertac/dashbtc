...
def fetch_social(keywords):
    """
    StockTwits JSON API — ücretsiz, auth gerekmez, symbol bazlı.
    keywords varsa → StockTwits symbol stream
    keywords yoksa → mevcut SYMBOL için stream
    """
    items  = []
    sym_base = SYMBOL.split("/")[0]

    # StockTwits symbol kodu
    st_sym = STOCKTWITS_MAP.get(sym_base, sym_base + ".X")

    # Keyword varsa arama, yoksa symbol stream
    try:
        if keywords:
            # StockTwits'te direkt arama endpoint'i yok;
            # Her keyword için en yakın symbol stream'ini dene
            kw = keywords[0].upper()
            st_sym = STOCKTWITS_MAP.get(kw, st_sym)
...
function drawChart(candles) {
  ...
  data.forEach((c, i) => {
    ...
    ctx2.beginPath();
    ctx2.moveTo(xc, Math.round(yH));
    ctx2.lineTo(xc, Math.round(bTop));
    ctx2.moveTo(xc, Math.round(bBot));
    ctx2.lineTo(xc, Math.round(yL));
    ctx2.stroke();

    // Gövde
    ctx2.fillStyle = col;
    const rx = Math.round(x - candleW / 2);
    const rw = Math.max(1, Math.round(candleW));
    const ry = Math.round(bTop);
    const rh = Math.max(1, Math.round(bBot) - ry);  // min 1px, asla 0 değil
