...
canvas{display:block;width:100%;min-height:0;flex:1}
...
function drawChart(candles) {
  ...
  // Gerçek piksel boyutunu al — offsetWidth/Height layout bitmeden de çalışır
  const wrap = cvs.parentElement;
  const W = wrap.offsetWidth  - 28;    // padding çıkarıldı
  const H = wrap.offsetHeight - 36;    // panel-title yüksekliği çıkarıldı

  if (W < 50 || H < 50) {
    // Boyut henüz hesaplanmadı — bir sonraki frame'de tekrar dene
...
// ResizeObserver: chart-wrap boyut aldığında veya değiştiğinde yeniden çiz
(function() {
  const wrap = cvs.parentElement;
  if (!wrap) return;
