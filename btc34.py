...
def generate_signals(price, bid_walls, ask_walls, df):
    ...
    def _build(direction, wp, wv, dist):
        ...
        return {
            ...
            "htf_trend"  : htf.get("trend", "NEUTRAL"),
            "block_reason": "hard block" if conf["hard"] else "",
            "mkt_score"  : conf["k4"],
        }
...
function renderSignals(d){
  ...
  html+=active.map(s=>{
    ...
    return`<div class="signal-box ${isLong?'long':'short'}" style="border-left-color:${mainClr}">
      ...
      <div style="font-size:9px;color:var(--text-dim);margin-bottom:5px">Net TP: <span style="color:var(--green)">+${s.net_tp_pct||0}%</span> · Net SL: <span style="color:var(--red)">-${s.net_sl_pct||0}%</span> · R/R: ${((s.net_tp_pct||0)/(s.net_sl_pct||1)).toFixed(2)}:1 · Duvar ${s.dist_pct||0}% uzakta</div>
