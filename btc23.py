    ...
    with db_connect() as conn:
        conn.execute("""
            CREATE TABLE IF NOT EXISTS signals (
                ...
                wall_vol    REAL,
                htf_trend   TEXT,
                status      TEXT    NOT NULL DEFAULT 'pending',  -- pending / win / loss / timeout
                outcome     TEXT,                      -- WIN / LOSS
                net_pnl_pct REAL,
                net_pnl_usd REAL,
                open_ts     TEXT    NOT NULL,
                close_ts    TEXT,
                checks_json TEXT                       -- JSON string
            )
        """)
        conn.commit()
    print(f"[DB] {DB_FILE}")
...
def db_close_signal(rowid, outcome, net_pnl_pct, net_pnl_usd, close_ts):
    """Sinyali kapat: outcome + P&L yaz."""
    status = outcome.lower()  # win / loss
    with db_connect() as conn:
        conn.execute("""
            UPDATE signals SET status=?, outcome=?, net_pnl_pct=?, net_pnl_usd=?, close_ts=?
            WHERE id=?
        """, (status, outcome, net_pnl_pct, net_pnl_usd, close_ts, rowid))
        conn.commit()
...
def generate_signals(price, bid_walls, ask_walls, df):
    ...
    if has_long and has_short:
        # Her iki yönde aktif sinyal var — sadece biri seçilir
        ...
        # Karar kriterleri (sırasıyla):
        # 1. HTF trendiyle uyumlu olan kazanır
        # 2. Piyasa verisi skoru (mkt_score) daha yüksek olan
        # 3. Genel skor daha yüksek olan
        # 4. Duvara daha yakın olan (dist_pct küçük = daha iyi)
        htf_trend = htf["trend"]
        ...
        loser_dir = "SHORT" if winner["dir"] == "LONG" else "LONG"
        print(f"[ÇAKIŞMA] {winner['dir']} kazandı (skor {winner['score']} vs {best_long['score'] if loser_dir=='LONG' else best_short['score']}) — {loser_dir} iptal")
        active = [winner]
...
def _close_signal(sig, outcome, net_pnl_pct, net_pnl_usd, close_ts):
    """Sinyali hem DB'ye hem RAM cache'e yaz."""
    ...
    if db_id:
        try:
            db_close_signal(db_id, outcome, net_pnl_pct, net_pnl_usd, close_ts)
        except Exception as e:
            print(f"[DB HATA] close_signal: {e}")
    print(f"[SİNYAL] {sig['dir']} {outcome} | PnL: {net_pnl_pct}% | DB id:{db_id}")
...
def check_pending_signals(df):
    ...
    for sig in _pending_signals:
        ...
        for _, row in future_candles.iterrows():
            ...
            _close_signal(sig, outcome, net_pnl_pct, net_pnl_usd, datetime.now().strftime("%H:%M:%S"))
            resolved = True; break
        ...
        if not resolved:
            if sig["waited_count"] >= MAX_CANDLES_WAIT:
                ...
                net_pnl_pct = round((TP_PCT-rt)*100,2)  if outcome=="WIN" else -round((SL_PCT+rt)*100,2)
                net_pnl_usd = round(sig["entry"]*(TP_PCT-rt),2) if outcome=="WIN" else -round(sig["entry"]*(SL_PCT+rt),2)
                _close_signal(sig, outcome, net_pnl_pct, net_pnl_usd, datetime.now().strftime("%H:%M:%S")+" (timeout)")
            else:
    ...
    while True:
        try:
            ...
            for sig in signals:
                ...
                if not already:
                    new_sig={**sig,"ts":datetime.now().strftime("%H:%M:%S"),"symbol":SYMBOL,"entry_candle_idx":len(df)-1,"waited_count":0}
                    try:
                        db_id=db_insert_signal(new_sig); new_sig["_db_id"]=db_id
                    except Exception as e:
                        print(f"[DB HATA] insert: {e}"); new_sig["_db_id"]=None
                    _pending_signals.append(new_sig)
                    print(f"[YENİ SİNYAL] {sig['dir']} @ {sig['entry']} Score:{sig['score']} DB:{new_sig['_db_id']}")
