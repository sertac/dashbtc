...
function drawChart(candles) {
  ...
  // Container'dan boyut al — her zaman güvenilir
  const container = document.getElementById('chart-container');
  const W = container.offsetWidth;
  const H = container.offsetHeight;
  if (!W || !H || W < 30 || H < 30) { setTimeout(()=>drawChart(candles),120); return; }

  // Her seferinde buffer'ı sıfırla (zorunlu, skipped draw'ı önler)
  cvs.width  = W;
  cvs.height = H;
...
function initChart(candles) {
  ...
  // Container boyut alana kadar bekle
  let tries = 0;
  const t = setInterval(() => {
    const el = document.getElementById('chart-container');
    if ((el && el.offsetWidth > 30 && el.offsetHeight > 30) || tries++ > 30) {
      clearInterval(t);
      drawChart(candles);
    }
  }, 80);
