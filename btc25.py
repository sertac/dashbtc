...
def _close_signal(sig, outcome, net_pnl_pct, net_pnl_usd, close_ts, exit_price=None):
    ...
    # Süre: sinyal açılış zamanından kapanışa kadar (dakika)
    ...
    print(f"[SİNYAL] {sig['dir']} {outcome} | PnL:{net_pnl_pct}% | Çıkış:{exit_price} | Süre:{duration_min}dk")
...
def check_pending_signals(df):
    ...
    for sig in _pending_signals:
        ...
        if tp_hit or sl_hit:
            ...
            # Süre: açılış zamanından itibaren geçen süre
            duration_min = None
            try:
                open_dt  = datetime.strptime(sig.get("ts","")[:8], "%H:%M:%S")
                close_dt = datetime.strptime(now_ts, "%H:%M:%S")
                delta    = (close_dt - open_dt).seconds // 60
                duration_min = delta
                sig["_duration_override"] = duration_min
            except Exception:
                pass
