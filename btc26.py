...
def background_loop():
    ...
    while True:
        try:
            ...
            for sig in signals:
                # Aynı symbol için zaten açık pending sinyal var mı?
                open_for_symbol = [p for p in _pending_signals if p.get("symbol") == SYMBOL]

                # Aynı yönde veya karşı yönde açık sinyal varsa ekleme
                if open_for_symbol:
                    same_dir = any(p["dir"] == sig["dir"] for p in open_for_symbol)
                    opp_dir  = any(p["dir"] != sig["dir"] for p in open_for_symbol)
                    if same_dir:
                        continue  # zaten bu yönde var
                    if opp_dir and not sig["htf_blocked"]:
                        # Karşı yönde açık sinyal var — sadece çok daha yüksek skorluysa değiştir
                        existing = open_for_symbol[0]
                        if sig["score"] <= existing["score"] + 1:
                            continue  # mevcut sinyali koru
                        # Mevcut sinyali pending'den çıkar, yenisini ekle
                        print(f"[GÜNCELLEME] {existing['dir']} sinyali {sig['dir']} ile değiştirildi (skor {sig['score']} > {existing['score']})")
